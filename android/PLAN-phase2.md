...
| 단계 | 산출물 | "동작한다"의 정의 |
|---|---|---|
| **2-A. 상태 배선** ✅ | `GameViewModel` + `StateFlow<GameUiState>`, `:feature:game`에서 도메인 호출 | 딜된 초기 상태가 로그/텍스트로 보이고, 버튼으로 `PlayCard`를 쏘면 상태가 갱신된다 — **완료(커밋 66057fb)** |
| **2-B. 정적 보드 렌더** ✅ | 8밴드 레이아웃 + 카드 렌더(Coil-SVG 로더), 좌표계 확정 | 딜 직후 보드가 실제 카드 이미지로 그려진다(입력 없음, read-only) — **완료(d3b6bf1)** |
| **2-C. 입력·상호작용** ✅ | 손패 선택(아크 팝업/히트박스 확장), 드래그→바닥 스냅, 더미 뒤집기 | 손으로 카드를 내고 바닥에 스냅되며 매칭/획득이 화면에 반영된다 — **완료(2458a32)** |
| **2-D. 오버레이·특수상황** ✅ | 고/스톱 모달, 특수상황 비파괴 스탬프/토스트(뻑/쪽/따닥/싹쓸이), 흔들기/폭탄 액션바 | 3점↑에서 고/스톱을 고르고, 특수상황이 발생 지점에 표시된다 — **완료(e9092ae)** |
| **2-E. 완주·결과** ✅ | 판 종료 3조건 처리 + 최소 결과 표시 + 새 딜 재시작 | 스톱/총통/나가리로 1판이 끝나고 다시 시작할 수 있다 — **완료(283e0a2)** |

> **Phase 2 완료(2026-07-08)**: 5단계 전부 구현. code-reviewer 검수로 빈손 턴 FlipOnly 데드락(H1)·더블탭 크래시(M1) 수정, qa-tester가 `:feature:game` 단위 테스트 21종 추가(전부 통과). 실기기 hotseat 1판 완주 육안 검증은 미실시(아래 §11).
...
- **흔들기/폭탄 활성 조건**: 손패 같은 월 3장 감지 → 액션바 버튼 활성 + 해당 3장 하이라이트(§4-2). `GameUiState.shakeableMonths/bombableMonths`로 노출.
  ...
  - ⚠️ **2-A 이월**: 현재 `shakeable/bombable/canGo`는 좌석 고정으로 **P1 관점 값**이다. 2-C에서 "액션바 활성·고 판정은 **현재 turn 플레이어 기준**"으로 재정의해야 한다(P2 턴 손패 공개 UX와 함께).
...
| # | 항목 | 영향 | 처리 |
|---|---|---|---|
| 0 | **좌석 정책** | 관점·밴드 배치 | ✅ **확정: 하단 = P1 고정**(상단 = P2, Phase 3에서 P2 자리에 AI). `toUiState(me=P1)` 고정. |
...
## 11. 실기기 육안 검증 필요 (Phase 2 잔여)

빌드·단위테스트(feature 21 / domain 59)는 통과했으나, 아래는 렌더·입력 특성상 **에뮬레이터/실기기에서 눈으로** 확인해야 하며 아직 미실시다:

1. **hotseat 1판 완주**: P1↔P2 교대(P2 패스앤플레이 게이트), 3점↑ 고/스톱 선택, 스톱 종료 → 결과 점수 내역(박/흔들기·폭탄 배수 표기 vs `FinalScore.total`) 정확성.
2. **종료 3조건 각 경로**: 총통(초기 딜/재딜)·나가리(끝까지 진행)·스톱 승리 결과 화면 진입.
3. **매칭 2장 팝업**: 같은 월 광/피 등 종류 다른 2장 상황 → 선택→획득 의도대로, 바깥 탭 취소 안전성.
4. **빈손 턴 자동 뒤집기(H1 수정 검증)**: 폭탄으로 손패를 먼저 소진시킨 뒤 판이 멈추지 않고 진행되는지.
5. **재시작 반복**: 여러 판 연속 시 오버레이/스탬프/flip/좌표·무장 상태 잔여 없이 초기화.
6. **소형 화면 레이아웃**: 바닥 넘침/클립, 손패 히트박스 정확도, 결과·팝업 넘침(§9 리스크 #4).

## 12. Phase 3~5로 이월된 것 (Phase 2 범위 밖 확정)

- **Phase 3(AI)**: P2는 현재 패스앤플레이(사람). `PassAndPlayOverlay` 블록만 제거하고 AI 자동 플레이로 대체(격리 완료).
- **Phase 4(손맛/사운드)**: 애니 수치(아크·겹침·셰이크·flip·트랜지언트 지속시간) 전부 잠정. `events: Flow<GameEvent>` 채널에 SFX/햅틱을 얹으면 됨. 이벤트 구독을 `repeatOnLifecycle`로 감싸는 것도 이때(리뷰 M2).
- **Phase 5(폴리시)**: 타이틀/일시정지/코치마크, 결과 카타르시스 연출(현재 결과 화면은 정보 최소), 설정 진입 흐름 최종 확정.
- **단순화 유지분(승인)**: 폭탄 슬램/싹쓸이 스윕/PiStolen 비행은 근사 연출 — Phase 4에서 정식화.
- **최적화 여지(리뷰 L2)**: Coil `ImageLoader` 앱 단일 재사용(DI) 검토.
